#!/bin/bash
#SBATCH --job-name=genius-fiq-medium-srchk-retrain
#SBATCH --output=/home/tcetoje/logs/genius_fiq_medium_srchk_retrain_%j.log
#SBATCH --error=/home/tcetoje/logs/genius_fiq_medium_srchk_retrain_%j.log
...
# Retrain-from-scratch (2026-07-03): medium's seedrepaircheck checkpoint (repaired quantizer,
...
TRAIN_CONFIG="$MODEL_DIR/configs_scripts/large/train/inbatch/inbatch_fashioniq_only_medium_seedrepaircheck.yaml"
EVAL_CONFIG="$MODEL_DIR/configs_scripts/large/eval/inbatch/config_eval_fashioniq_only_medium_seedrepaircheck.yaml"
NPROC=4
...
echo "##### Retraining: medium seedrepaircheck (fresh DDP run) #####"
cd "$COMMON_DIR"
...
STATUS=$?
if [ $STATUS -ne 0 ]; then
    echo "ERROR: medium seedrepaircheck retrain failed with exit code $STATUS"
    exit $STATUS
fi
...
STATUS=$?
if [ $STATUS -ne 0 ]; then
    echo "ERROR: medium seedrepaircheck eval (post-retrain) failed with exit code $STATUS -- STOP, do not retry again blindly, escalate to user."
    exit $STATUS
fi
