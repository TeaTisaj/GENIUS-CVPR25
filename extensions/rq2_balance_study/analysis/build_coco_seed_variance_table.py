"""
Seed-variance check (RQ3 robustness addition): compares COCO Recall@1 across 3
training seeds (2023 [real study, reused -- NOT retrained here], 7, 13) per variant, to
check whether the reported ranking (vanilla -> weak up -> medium down -> strong up +50.9%)
survives training-seed noise -- mirrors build_seed_variance_table.py (FashionIQ), which
already showed the FashionIQ weak/medium/strong ranking does NOT survive seed noise.
...
Run with: PYTHONPATH=<repo>/src/common:<repo>/src python build_coco_seed_variance_table.py
"""
import argparse
import csv
import os
...
def main():
    ...
    parser.add_argument("--table2_csv", required=True,
                         help="Real study's table2_retrieval_performance.csv (COCO)")
