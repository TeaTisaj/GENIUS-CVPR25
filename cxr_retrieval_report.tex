...
\begin{abstract}
This report describes an internship project investigating the application of
\emph{generative retrieval} to fine-grained chest X-ray (CXR) case retrieval.
The central research question is not simply whether the GENIUS framework can be
adapted to medical data, but whether the generative retrieval paradigm
- which replaces nearest-neighbour search with autoregressive decoding of
discrete document identifiers - can learn \emph{discriminative} semantic IDs
for a domain where images are globally homogeneous and clinically meaningful
variation is subtle.

We use BiomedCLIP as a domain-appropriate encoder and retrain both the
...
search prior cases with similar findings to support diagnostic reasoning.
...
embeddings --- which returns visually or semantically similar cases as ranked
by cosine distance in some representation space.
...
identifier string for the most relevant candidate given a query.
This collapses retrieval to a sequence generation problem, avoids the linear
scaling of index search with corpus size, and in principle encodes diagnostic
reasoning rather than surface similarity.

Chest X-rays present an especially demanding testbed for this paradigm:
images are globally similar (all show lungs, ribs, and soft tissue in largely
fixed geometry), while clinically meaningful differences are subtle ---
a small consolidation, an asymmetric pleural effusion, a widened mediastinum.
This creates a fundamental tension with how the residual quantizer constructs
identifiers: codes learned from global image statistics may not separate
cases that a radiologist would consider distinct.
...
Instead, the generative retrieval paradigm itself is under investigation:
we ask whether discrete identifier learning can achieve fine-grained
discrimination in a clinically demanding, low-visual-variance domain.
The answer shapes the contribution.
If the standard pipeline succeeds, this constitutes a positive result for
generative retrieval in medical search.
If it fails --- identifiers collapse to coarse disease categories, codebook
utilisation degrades, or retrieval quality falls short of a dense baseline ---
the contribution lies in diagnosing the failure mode and proposing targeted
...
Given a query consisting of a chest X-ray image $q_i$ and optional clinical
context $q_t$ (e.g.\ a brief indication or prior finding), retrieve a ranked
list of relevant radiology studies from a fixed corpus $\mathcal{C}$.

Following GENIUS notation, the system generates a discrete identifier
$T_c = (t^c_1, \ldots, t^c_M)$ for the most relevant candidate $c$:
\[
t^c_k = \arg\max_{t \in \mathcal{V}} \left[
    \log p\!\left(t \;\middle|\; \mathbf{q},\, t^c_{<k};\, \theta\right)
\right],
\]
where $\mathcal{V}$ is the codebook vocabulary and $\mathbf{q}$ is the query
embedding.
Candidate generation is constrained by a trie over valid identifier sequences,
ensuring the decoder can only produce IDs that exist in the candidate pool.
...
\begin{itemize}[itemsep=2pt]
    ...
    \item \textbf{Query modality}: \texttt{image} (image-only query at test time,
    reflecting realistic clinical use where a clinician searches by image).
\end{itemize}

Multimodal candidates (\texttt{image,text}) are used rather than text-only
(\texttt{image$\to$text}, MBEIR task~3): combining the frontal image and report
provides richer quantizer supervision, and the image component is already
available at retrieval time.
The resulting M-BEIR task type is \texttt{image$\to$image,text} (task~9),
which was absent from the original \texttt{MBEIR\_TASK} mapping in
\texttt{utils.py}; the entry has been added.
Note that query modality is \texttt{image} only (\texttt{query\_txt = None})
--- this is intentional and distinct from task~7 (\texttt{image,text$\to$image}),
which requires a text component in the query.

\subsection{Research questions}

\textbf{Primary:}
Can the generative retrieval paradigm, adapted to the medical domain via a
biomedical vision-language encoder, learn sufficiently discriminative discrete
identifiers for fine-grained CXR case retrieval?

\textbf{Secondary:}
\begin{itemize}[itemsep=2pt]
    \item Does the trained system generalise across datasets
    (MIMIC-CXR $\to$ IU~X-Ray), or does it overfit to MIMIC-CXR-specific
    acquisition and reporting conventions?
    \item How does generative retrieval compare to an embedding-based baseline
    using the same BiomedCLIP encoder?
    \item What are the failure modes of residual quantization under visual
    homogeneity? Does codebook collapse occur, and at what granularity?
    \item If the standard pipeline is insufficient, which training signal ---
    report-aware, disease-aware, or hard-negative-aware --- most effectively
    improves identifier discriminability?
\end{itemize}
...
candidate is considered positive for a given query if the two studies share at
...
Its main limitation --- it is insensitive to severity and laterality --- is
...
\textbf{Baseline.}
The primary comparison is an embedding-based retrieval system using the same
frozen BiomedCLIP encoder: query and candidate embeddings are compared by
cosine similarity, with no learned quantization or decoder.
This isolates the contribution of the generative retrieval framework from the
representation quality of the encoder.
